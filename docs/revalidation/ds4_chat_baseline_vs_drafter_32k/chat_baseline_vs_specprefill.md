...
> **Superseded diagnostic:** this run used baseline warm KV reuse against
> drafter `--spec-prefill-cache fresh`. That means baseline follow-up turns
> only synced the new suffix, while drafter invalidated KV and recompressed the
> full logical transcript every turn. The first-turn comparison is meaningful;
> follow-up TTFT and wall-time comparisons are apples-to-oranges. Do not use
> this chart as the actual chat-loop baseline-vs-SpecPrefill result.

Both modes ran the same prompt sequence while the logical conversation context
grew, but they did not use the same cache semantics after turn 1.
...
- This diagnostic used SpecPrefill `fresh` mode, which recompresses the full canonical transcript each turn, then syncs the compressed target view.
